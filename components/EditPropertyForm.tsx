...
interface EditPropertyFormProps {
    closeEditPropertyActionSheet: () => void;
    property: any; // The property to edit
}
...
export default function EditPropertyForm({ closeEditPropertyActionSheet, property }: EditPropertyFormProps) {
    const token = useAuthStore((state) => state.token);
    const [loading, setLoading] = useState(false);
    const [images, setImages] = useState<{ uri: string; file?: File; isExisting?: boolean; key?: string }[]>([]);
    ...
    // Initialize form data with the property to edit
    const [formData, setFormData] = useState<PropertyFormData>({
        title: property.title || '',
        description: property.description || '',
        propertyType: property.propertyType || 'full-house',
        price: {
            basePrice: property.price?.basePrice || 0,
            billType: property.price?.billType || 'monthly',
            securityDeposit: property.price?.securityDeposit || 0
        },
        location: {
        ...
        details: {
            roomType: property.details?.roomType || 'full-house',
            totalRooms: property.details?.totalRooms || 1,
            availableRooms: property.details?.availableRooms || 1,
            sharedBathroom: property.details?.sharedBathroom || false,
            furnishingStatus: property.details?.furnishingStatus || 'unfurnished',
            roomSize: property.details?.roomSize || 0,
            floorNumber: property.details?.floorNumber || 0,
        ...
        maintainenceCharges: {
            amount: property.maintainenceCharges?.amount || 0,
            billType: property.maintainenceCharges?.billType || 'monthly',
            includesFood: property.maintainenceCharges?.includesFood || false,
            includesUtility: property.maintainenceCharges?.includesUtility || false
        }
    });
    ...
    useEffect(() => {
        ...
        if (property.amenities) {
            ...
            const custom = property.amenities.filter(
                (amenity: string) => !standardAmenities.includes(amenity)
            );
        ...
        try {
            ...
            if (address.length > 0) {
                ...
                setFormData(prev => ({
                    ...
                    location: {
                        ...prev.location,
                        address: `${currentAddress.street || ''} ${currentAddress.district || ''}`.trim(),
    ...
    const handleSubmit = async () => {
        try {
            ...
            const updatedData = {
                ...formData,
                // Convert availableFrom to ISO string if it's a Date object
                availableFrom: formData.availableFrom instanceof Date
            ...
            // Format new images for upload (only non-existing ones)
            ...
            console.log("Sending new images:", newImages);
            ...
            Alert.alert('Success', 'Property updated successfully', [
                {
                    text: 'OK',
                    onPress: closeEditPropertyActionSheet
                }
            ]);
        } catch (error: any) {
        ...
        <ScrollView className='px-3 py-2'>
            ...
            <View className='flex-row items-center justify-between mb-4'>
                <View className='flex-row items-center gap-2'>
                    <TouchableOpacity onPress={closeEditPropertyActionSheet}>
            ...
            <View className='mb-6'>
                <Button
                    text={loading ? "Updating Property..." : "Update Property"}
